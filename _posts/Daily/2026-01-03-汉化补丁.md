---
title: Update for the Chinese patch of PCSG00543
key: Chinese_patch
tags: ["Daily"]
aside:
  toc: true
---

PCSG00543 汉化补丁更新 <!--more-->

**通知:**

```
开启内部测试，欢迎进群反馈 BUG 交流
QQ 群号:961232699
```

# 前言

本补丁为《路人女主的养成方法 ~blessing flowers~》PSV 版（PCSG00543）非官方汉化补丁。

我很高兴参与路人女主 PSV 游戏的汉化，项目持续优化中，欢迎通过评论区反馈问题。

汉化补丁的更新帖，最新版本见文末。

# 更新日志

## v1.5 (26-01-02-v1.5)

### 更新内容

- 修复了新版`tools\dat_recursive.py`引入的内存对齐问题，`slot_newicon`图标已经可以正常显示
- 修正了`sa2084ess0.txt`中的文本，解决了对应选项的问题
- ~~开源部分脚本与编译批处理文件以供参考（很多代码都不是我一个人的功劳，感谢团队合作）具体见[PCSG00543-CN-tools](https://github.com/Zucker-jex/PCSG00543-CN-tools)~~ 原因见末尾

## v1.4 (25-11-08-v1.4)

### 更新内容

- 修复了由于字符错误导致 11 月 23 日的报错
- 非常感谢[leec0m3t](https://github.com/leec0m3t)与[21-ko](https://github.com/21-ko)韩化团队成员的帮助，没有他们我难以发觉这些错误

## v1.3 (25-02-24-v1.3)

### 更新内容

- 修正了部分剧情文本的内容，解决文本语音错位问题

### 待解决

- 仍然无法完美解决内存溢出的问题

## v1.2 (24-08-07-v1.2)

### 更新内容

- 修正了`sa0990ed10.txt`的内容，解决该结局的报错

### 待解决

- 仍然无法完美解决内存溢出的问题

## v1.1 (24-02-15-v1.1)

### 完全汉化

太累了。。。但我很满意

### 进展

- 加入了图片校验转换机制
- 解析了 csv 的内存分布表
- 更改伪字库域为全局文本，不包括 csv 文件
- 移除了 Herobrine

## v0.2t (24-01-28-v0.2t)

### 未完全汉化，仅供测试

### 进展

- 加入了自动化伪字库生成与 ttf 修改
- 完善了绝大部分代码逻辑（请使用相对路径指向文件）
- 修正了 first.dat
- 加入了 log 以供检查
- 移除了 Herobrine

# 使用

## 实体机

- 安装 repatch 插件
- 解压至\ux0\rePatch\PCSG00543\PSP2_GAME\USRDIR

## vita3k

- 解压后覆盖粘贴至 vita3k\psv\ux0\app\PCSG00543\PSP2_GAME\USRDIR

# 部分源码提供与参考

- [vitasdk](https://github.com/vitasdk/)
  - [vita-headers](https://github.com/vitasdk/vita-headers)
- [caoyang131](https://github.com/caoyang131/)
  - [TextTool](https://github.com/caoyang131/TextTool)
  - DatTool
- [xdanieldzd](https://github.com/xdanieldzd/)
  - [GXTConvert](https://github.com/xdanieldzd/GXTConvert)
  - [Scarlet](https://github.com/xdanieldzd/Scarlet)
- [ningshanwutuobang](https://github.com/ningshanwutuobang/)
  - [ChaosChildPCTools](https://github.com/ningshanwutuobang/ChaosChildPCTools)

# 关于工具链的说明

出于 **项目安全、法律风险防控及防止技术滥用** 的考虑，本次汉化所使用的完整工具链将 **保持内部维护，暂不开源**。
我们坚信，汉化项目的核心价值在于为玩家提供 **高质量、负责任的文化输出**，而非技术本身。我们也呼吁所有爱好者尊重游戏版权，支持正版。

虽然工具链不会公开，但我们始终欢迎基于 **技术学习与交流** 目的的友好讨论。如果您在自制汉化项目中遇到类似的技术问题，我们仍乐意在力所能及的范围内提供思路参考。

最后，再次感谢每一位为这个项目付出过努力的人。享受游戏吧！

# 最新汉化补丁

- [26-01-02-v1.5.7z](https://zuckertech-my.sharepoint.com/:u:/g/personal/jex_zuckertech_onmicrosoft_com/IQBmsEgfFmQPRatOtPNLJW83Acw6C9Ur2w1SuCJYtl6oqe4?e=3zz5rt)
